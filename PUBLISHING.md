...
2. (선택) 테스트용 [TestPyPI](https://test.pypi.org)에도 동일 계정으로 가입
...
3. **테스트 업로드(선택)**: `twine upload --repository testpypi dist/*`  
   설치 테스트: `pip install -i https://test.pypi.org/simple/ elio-mcp-server`
4. **실제 PyPI 업로드**: `twine upload dist/*` (토큰 입력 또는 `.pypirc` 사용)

한 번 업로드한 버전은 PyPI에서 수정·삭제가 제한되므로, 테스트는 가능하면 TestPyPI에서 하세요.
