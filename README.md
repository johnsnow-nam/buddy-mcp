...
| 파일 | 용도 |
|------|------|
| **`.cursor/mcp.json`** | Cursor에서 이 프로젝트 열 때 사용. `command` + `args`(run.py 절대 경로)로 **run.py**를 실행하면 Mac/Windows/Linux 모두 가상환경으로만 동작합니다. |
| **`mcp-config.example.json`** | 다른 클라이언트에 복사해 쓸 예시. `command`는 `python3` 또는 그 PC의 Python 경로, `args`는 **run.py 절대 경로**로 바꾸세요. |

**설정에 넣을 JSON**은 프로젝트 폴더에서 `python3 run.py --print-mcp-config` 를 실행해 나온 내용을 쓰면 됩니다.

### Cursor

- 이 프로젝트를 열면 **`.cursor/mcp.json`** 이 적용됩니다.
- 다른 PC나 사용자 환경에서는 해당 위치에서 한 번 `python3 run.py --print-mcp-config` 를 실행해 나온 JSON으로 `.cursor/mcp.json`을 갱신하면 됩니다.
